...
public class HttpClientUtil {
    ...
    public static String createTask(String[] results, String credentialsUser, String listId) throws IOException, InterruptedException {
        ...
        int passedCount = 0;
        int failedCount = 0;
        Set<String> failedResults = new LinkedHashSet<>();
        Set<String> passedResults = new LinkedHashSet<>();

        // Process each test result
        for (String result : results) {
            String formattedResult = formatTestResult(result);
            if (result.contains("Passed")) {
                passedResults.add(formattedResult);
                passedCount++;
            } else if (result.contains("Failed")) {
                failedResults.add(formattedResult);
                failedCount++;
            }
        }
        ...
        // Calculate the success ratio in percentage
        double successRatio = totalTests > 0 ? (passedCount * 100.0 / totalTests) : 0;

        StringBuilder markdownDescription = new StringBuilder();
        markdownDescription.append(String.format("Total tests executed: %d\nFailed: %d / Passed: %d\n", totalTests, failedCount, passedCount));
        markdownDescription.append(String.format("Success Ratio: %.2f%%\n\n", successRatio)); // Add Success Ratio

        // Add grouped test results
        markdownDescription.append("Test Results:\n");

        // Add failed results first
        for (String failedResult : failedResults) {
            markdownDescription.append(failedResult).append("\n");
        }

        // Add a blank line between failed and passed results
        if (!failedResults.isEmpty() && !passedResults.isEmpty()) {
            markdownDescription.append("\n");
        }

        // Add passed results
        for (String passedResult : passedResults) {
            markdownDescription.append(passedResult).append("\n");
        }
